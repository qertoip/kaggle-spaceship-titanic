...
def new_model():
    return CatBoostClassifier(
        l2_leaf_reg=12, n_estimators=400, #rsm=0.8,
        random_state=1013, silent=True
    )

    # 80.93%
    # return CatBoostClassifier(
    #     l2_leaf_reg=6, n_estimators=300, # rsm=0.8, n_estimators=100,
    #     random_state=1013, silent=True
    # )
...
def extract_features_df(df: DataFrame) -> DataFrame:
    ...
    #df['AgeLog10'] = np.log10(df['Age'] + 1)
    # df['Age_0-12'] = (df['Age'] <= 12).astype(int)
    # df['Age_13-17'] = ((13 <= df['Age']) & (df['Age'] <= 17)).astype(int)
    # df['Age_18-25'] = ((18 <= df['Age']) & (df['Age'] <= 25)).astype(int)
    # df['Age_26-30'] = ((26 <= df['Age']) & (df['Age'] <= 30)).astype(int)
    # df['Age_31-50'] = ((31 <= df['Age']) & (df['Age'] <= 50)).astype(int)
    # df['Age_51+'] = (51 <= df['Age']).astype(int)
    # df.drop(columns='Age', inplace=True)

    # df['Age_0-17'] = ((0 <= df['Age']) & (df['Age'] <= 17)).astype(int)
    # df['Age_18-25_and_31-50'] = (((18 <= df['Age']) & (df['Age'] <= 25)) | ((31 <= df['Age']) & (df['Age'] <= 50))).astype(int)
    # df['Age_26-30_and_51+'] = (((26 <= df['Age']) & (df['Age'] <= 30)) | (51 <= df['Age'])).astype(int)
    # df.drop(columns='Age', inplace=True)
